...
export function InfoNode({ data, selected }: NodeProps<LaborNodeData>) {
  return (
    <div
      className={`rounded-xl border-2 px-4 py-3 min-w-[140px] max-w-[200px] bg-gray-100 ${
        selected ? 'border-gray-500 shadow-lg shadow-gray-200' : 'border-gray-400'
      }`}
    >
      <p className="text-xs font-semibold text-gray-700 leading-tight text-center">{data.label}</p>
